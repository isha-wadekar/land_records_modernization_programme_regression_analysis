...
vignette("dplyr")
hist(pv_map, breaks=36, col='cyan4', main='Number Maps per Village', xlab='Number Maps per
Village')
hist(pv_goodmap, breaks=36, col='cyan4', main='Good Condition Maps per Village', xlab='Good
Condition Maps per Village')
hist(pv_digitizedmap, breaks=36, col='cyan4', main='Digitized Maps per Village', xlab='Digitized
Maps per Village')
#1.2]scatter plot with pv_map in the y-axis and compsite_points in the x-axis
library(ggplot2)
ggplot(gidi, aes(x=composite_points, y=pv_map))+
geom_point()+geom_smooth(method="lm")+ggtitle('Average number of maps to composite
points')
#1.3]run regression
m1<-lm(pv_map~composite_points+total_villages,data=gidi)
summary(m1)

plot(pv_map~composite_points+total_villages,data=gidi, main='Number of maps per village VS
Composite_points')
abline(m1)
#1.6] regress for pv_goodmap and pv_digitizedmap
m2=lm(pv_goodmap~composite_points+total_villages,data=gidi)
summary(m2)
plot(pv_goodmap~composite_points+total_villages,data=gidi,main='Good condition maps per
village VS Composite_points')
abline(m2)
